                        ...
                        <tbody>
                            @forelse($driverPerformance as $index => $driver)
                                <tr class="hover">
                                    ...
                                    <td>
                                        <span
                                            class="badge badge-success badge-sm">{{ $driver->completed_deliveries }}</span>
                                    </td>
                                    ...
                                    <td>
                                        ...
                                        <div class="flex items-center gap-2">
                                            <progress class="progress progress-success w-16" value="{{ $effectiveness }}"
                                                max="100"></progress>
                    ...
                    @forelse($statusDistribution as $status)
                        ...
                        <div>
                            <div class="flex justify-between items-center mb-2">
                                <div class="flex items-center gap-2">
                                    <span
                                        class="badge {{ $badgeClass }} badge-sm">{{ $status->estadoEnvio->nombre ?? 'N/A' }}</span>
                                    <span class="font-semibold">{{ $status->total }}</span>
                                </div>
                                <span class="text-sm text-base-content/70">{{ $percentage }}%</span>
                            </div>
                            <progress class="progress {{ $colorClass }} w-full" value="{{ $percentage }}"
                                max="100"></progress>
                        </div>
                    @empty
                        ...
                        @forelse($deliveries as $delivery)
                            <tr class="hover">
                                ...
                                <td>
                                    ...
                                    <div class="text-xs text-base-content/70">Peso: {{ $delivery->paquete->peso ?? 'N/A' }}
                                        kg</div>
                                </td>
                                <td>
                                    <div class="flex items-center gap-2">
                                        <div>
                                            ...
                                            <div class="text-xs text-base-content/70">
                                                {{ $delivery->motorista->telefono ?? 'N/A' }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td>
                                    ...
                                    @endphp
                                    <span
                                        class="badge {{ $badgeClass }} rounded-full">{{ $delivery->estadoEnvio->nombre ?? 'N/A' }}</span>
                                </td>
                                <td>
                                    <div class="text-sm">
                                        {{ \Carbon\Carbon::parse($delivery->fecha_estimada)->format('d/m/Y') }}</div>
                                </td>
                                <td class="font-mono font-semibold text-success">${{ number_format($delivery->costo, 2) }}
                                </td>
                                <td>
                                    <button wire:click="viewHistory({{ $delivery->id }})"
                                        class="btn btn-sm btn-ghost btn-circle" title="Ver Historial">
                                    ...
                                    <li class="step step-primary">
                                        <div class="text-left w-full ml-4 mb-4">
                                            <div class="flex items-center gap-2 mb-1">
                                                <span
                                                    class="font-bold">{{ $record->estadoEnvio->nombre ?? 'Estado Desconocido' }}</span>
                                                <span
                                                    class="text-xs text-base-content/60">{{ \Carbon\Carbon::parse($record->created_at)->format('d/m/Y h:i A') }}</span>
                                            </div>
                                            ...
                                            @if($record->foto_url)
                                                <div class="mt-2">
                                                    <button wire:click="viewImage('{{ $record->foto_url }}')"
                                                        class="btn btn-xs btn-outline btn-primary gap-2">
        ...
        <div class="modal modal-open" style="z-index: 9999;">
            <div class="modal-box relative max-w-4xl flex justify-center items-center bg-transparent shadow-none">
                <button wire:click="closeImageModal"
                    ...
                    <img src="{{ asset('storage/' . $selectedImageUrl) }}" alt="Evidencia Grande"
