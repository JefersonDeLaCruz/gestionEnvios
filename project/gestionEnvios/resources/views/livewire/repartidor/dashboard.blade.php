    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <div class="stats shadow">
            <div class="stat">
                ...
                <div class="stat-desc">De {{ $stats['total_assigned'] }} totales</div>
            </div>
        </div>
        ...
        <div class="stats shadow">
            <div class="stat">
                ...
                <div class="stat-value text-warning">{{ $stats['pending_today'] }}</div>
        ...
        <div class="stats shadow">
            <div class="stat">
                ...
                <div class="stat-value text-accent">{{ \Carbon\Carbon::now()->format('H:i') }}</div>
    ...
    <div class="card bg-base-100 shadow-xl mb-6">
        <div class="card-body">
            ...
            <div class="flex items-center gap-4">
                <div class="flex-1">
                    <progress class="progress progress-success w-full h-4" value="{{ $stats['completion_rate'] }}" max="100"></progress>
                </div>
                <span class="font-bold text-lg">{{ $stats['completion_rate'] }}%</span>
            </div>
            <div class="text-sm text-base-content/70">{{ $stats['completed_today'] }} de {{ $stats['total_assigned'] }} paquetes entregados</div>
        </div>
    </div>
        ...
        <div class="card bg-base-100 shadow-xl">
            <div class="card-body">
                ...
                <div class="space-y-3 mt-4">
                    @forelse($upcomingDeliveries as $index => $envio)
                    @php
                    ...
                    <div class="flex items-start gap-3 p-3 bg-base-200 rounded-lg hover:bg-base-300 cursor-pointer transition-all">
                        <div class="badge {{ $badgeColor }} badge-lg">{{ $index + 1 }}</div>
                        <div class="flex-1">
                            <div class="font-semibold">#{{ $envio->paquete->codigo }} - {{ $receptor?->nombre }} {{ $receptor?->apellido }}</div>
                            <div class="text-sm text-base-content/70">{{ $receptor?->direccion }}</div>
                            <div class="text-xs text-base-content/60 mt-1">
                                Estado: <span class="badge badge-xs">{{ $envio->estadoEnvio->nombre }}</span>
                            </div>
                        </div>
                ...
                <div class="card-actions justify-end mt-4">
                    <a href="{{ route('repartidor.packages') }}" class="btn btn-primary btn-sm gap-2">
            ...
            <div class="card-body">
                ...
                <div class="overflow-x-auto">
                    <table class="table table-sm">
                        ...
                        <tbody>
                            @forelse($recentHistory as $envio)
                            @php
                            ...
                            @endphp
                            <tr>
                                <td class="text-xs">{{ $envio->updated_at->format('h:i A') }}</td>
                                <td class="font-mono text-xs">#{{ $envio->paquete->codigo }}</td>
                                <td><span class="badge rounded-full {{ $badgeClass }} badge-xs">{{ $envio->estadoEnvio->nombre }}</span></td>
                            </tr>
                            @empty
                            <tr>
                ...
                <div class="text-sm text-base-content/70 mt-2 text-center">
                    Mostrando las últimas {{ $recentHistory->count() }} entregas
                </div>
                @endif
            </div>
        </div>
